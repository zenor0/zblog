...
describe('dev reference registry', () => {
  ...
  it('groups experimental pages separately from foundation references', () => {
    ...
    expect(experiments?.items.map((item) => item.slug)).toContain('article-title-layouts')
    expect(experiments?.items.map((item) => item.slug)).toContain('site-shell-candidates')
    ...
    expect(getDevReferenceItem('article-title-layouts')?.href).toBe('/dev/article-title-layouts')
    expect(getDevReferenceItem('site-shell-candidates')?.href).toBe('/dev/site-shell-candidates')
