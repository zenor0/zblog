...
function syncTitleOverflow(itemElement: HTMLLIElement, classNamePrefix: string) {
  const titleElement = itemElement.querySelector<HTMLElement>(`.${classNamePrefix}-map__title`)
  const titleTextElement = itemElement.querySelector<HTMLElement>(
    `.${classNamePrefix}-map__title-text`,
  )
    ...
    if (tocListElement) {
      ...
      const headingPoints = displayedHeadings.reduce<TocPoint[]>((points, heading, index) => {
        ...
        if (shouldSyncTitleOverflowRef.current) {
          syncTitleOverflow(itemElement, classNamePrefix)
        }

        const itemRect = itemElement.getBoundingClientRect()
        const titleElement = itemElement.querySelector<HTMLElement>(
          `.${classNamePrefix}-map__title`,
        )
