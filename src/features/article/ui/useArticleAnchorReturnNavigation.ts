...
} from '@/features/article/ui/article-anchor-navigation-utils'
...
export type ArticleAnchorReturnPosition = {
  left: number
  top: number
}

export type ArticleAnchorReturnState = {
  position: ArticleAnchorReturnPosition
  targetID: string
  top: number
}
...
export function resolveArticleAnchorReturnButtonPosition(
  target: HTMLElement,
  ...
  return {
    ...
    top: Math.round(top),
  } satisfies ArticleAnchorReturnPosition
}

export function useArticleAnchorReturnNavigation() {
  ...
  const [returnState, setReturnState] = useState<null | ArticleAnchorReturnState>(null)
  ...
  const dismissReturnState = useCallback(() => {
    clearHighlight()
  ...
  const revealReturnState = useCallback(
    (target: HTMLElement, top: number) => {
      ...
      setReturnState({
        position: resolveArticleAnchorReturnButtonPosition(target, buttonRef.current),
        targetID: target.id,
        top,
      })
  ...
  const syncReturnPosition = useCallback(() => {
    setReturnState((current) => {
      ...
      const position = resolveArticleAnchorReturnButtonPosition(target, buttonRef.current)
  ...
  const returnToReadingPosition = useCallback(() => {
    ...
    setReturnState(null)
  }, [clearHighlight, clearScrollObservation, returnState])
  ...
  const returnTargetID = returnState?.targetID
  ...
  }, [returnTargetID, syncReturnPosition])

  const returnButtonStyle: CSSProperties | undefined = returnState
    ? {
